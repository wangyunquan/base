...
public class DhtBucket {
    ...
    private final TreeSet<DhtContact> contacts;

    /* A set of last seen contacts that can replace any current contact that is unresponsive */
    private final TreeSet<DhtContact> replacementCache;
     ...
     * @param config
     */
    public DhtBucket(int depth, DhtConfiguration config)
    {
        this.depth = depth;
        this.config = config;
    }

    public synchronized void insert(DhtContact c)
    {
        if (this.contacts.contains(c))
        {
             ...
             */
            DhtContact tmp = this.removeFromContacts(c.getNode());
            tmp.setSeenNow();
            tmp.resetStaleCount();
        ...
        else
        {
            ...
            if (contacts.size() >= this.config.k())
            {
                ...
                DhtContact stalest = null;
                for (DhtContact tmp : this.contacts)
                {
    ...
    public synchronized void insert(DhtNode n)
    {
        this.insert(new DhtContact(n));
    }

    public synchronized boolean containsContact(DhtContact c)
    {
        return this.contacts.contains(c);
    }

    public synchronized boolean containsNode(DhtNode n)
    {
        return this.containsContact(new DhtContact(n));
    }

    public synchronized boolean removeContact(DhtContact c)
    {
        ...
        if (!this.replacementCache.isEmpty())
        {
            ...
            DhtContact replacement = this.replacementCache.first();
    ...
    private synchronized DhtContact getFromContacts(DhtNode n)
    {
        for (DhtContact c : this.contacts)
        {
    ...
    private synchronized DhtContact removeFromContacts(DhtNode n)
    {
        for (DhtContact c : this.contacts)
        {
            if (c.getNode().equals(n))
    ...
    public synchronized boolean removeNode(DhtNode n)
    {
        return this.removeContact(new DhtContact(n));
    }
    ...
    public synchronized List<DhtContact> getContacts()
    {
        final ArrayList<DhtContact> ret = new ArrayList<>();
     ...
     */
    private synchronized void insertIntoReplacementCache(DhtContact c)
    {
        ...
        if (this.replacementCache.contains(c))
        {
             ...
             * We need to remove and re-add the contact to get the Sorted Set to update sort order
             */
        	DhtContact tmp = this.removeFromReplacementCache(c.getNode());
            tmp.setSeenNow();
    ...
    private synchronized DhtContact removeFromReplacementCache(DhtNode n)
    {
        for (DhtContact c : this.replacementCache)
        {
            if (c.getNode().equals(n))
            {
    ...
    public synchronized String toString()
    {
        ...
        sb.append("\n Nodes: \n");
        for (DhtContact n : this.contacts)
        {
            sb.append("Node: ");
